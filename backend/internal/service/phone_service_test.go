...
func TestPurchasePhone_NotFound(t *testing.T) {
	svc := NewPhoneService(newMockPhoneRepo())
	err := svc.PurchasePhone(999, 1)
	if err == nil {
		t.Fatal("expected error for purchasing non-existent phone")
	}
}

func TestPurchasePhone_InsufficientStock(t *testing.T) {
	repo := newMockPhoneRepo()
	created := repo.Create(model.Phone{Brand: "Apple", Price: 999, Stock: 2})
	svc := NewPhoneService(repo)

	err := svc.PurchasePhone(created.ID, 5)
	if err == nil || err.Error() != "insufficient stock" {
		t.Fatalf("expected 'insufficient stock', got %v", err)
	}
}

func TestPurchasePhone_ReducesStock(t *testing.T) {
	repo := newMockPhoneRepo()
	created := repo.Create(model.Phone{Brand: "Apple", Price: 999, Stock: 10})
	svc := NewPhoneService(repo)

	if err := svc.PurchasePhone(created.ID, 3); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	phone, _ := repo.GetByID(created.ID)
	if phone.Stock != 7 {
		t.Errorf("expected stock 7 after purchasing 3, got %d", phone.Stock)
	}
}

func TestPurchasePhone_ExactStock(t *testing.T) {
	repo := newMockPhoneRepo()
	created := repo.Create(model.Phone{Brand: "Apple", Price: 999, Stock: 5})
	svc := NewPhoneService(repo)

	if err := svc.PurchasePhone(created.ID, 5); err != nil {
		t.Fatalf("unexpected error purchasing exactly available stock: %v", err)
	}
	phone, _ := repo.GetByID(created.ID)
	if phone.Stock != 0 {
		t.Errorf("expected stock 0, got %d", phone.Stock)
	}
}
